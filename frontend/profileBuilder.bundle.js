  ...
  var require_routingLabProfileBuilder = __commonJS({
    "frontend/routingLabProfileBuilder.js"() {
      ...
      var defaultPriorities = [
        ...
        { id: "latency", label: "Latency", subtitle: "Response speed", weight: 0.2, accent: siteColors.latency }
      ];
      ...
      var NodeCard = ({ title, subtitle, accent, onClick, active, nodeRef }) => {
          ...
          edge.type === "priority" ? [
            ...
            React.createElement("div", {
              ...
              React.createElement("button", {
                ...
                className: `rounded-full px-3 py-1 text-xs font-semibold transition ${edge.enabled ? "bg-[rgba(181,103,71,0.15)] text-[#5b2a1a]" : "bg-[rgba(92,49,30,0.1)] text-[rgba(43,29,20,0.7)]"}`
              }, edge.enabled ? "Enabled" : "Disabled")
            ])
          ]
        ]);
      };
      var FamilyDrawer = ({ family, onClose, onToggle, onBlacklist }) => {
        ...
        return React.createElement("aside", {
          ...
          React.createElement("div", {
            ...
            React.createElement("button", {
              ...
              className: `rounded-full px-3 py-1 text-xs font-semibold transition ${family.enabled ? "bg-[rgba(181,103,71,0.15)] text-[#5b2a1a]" : "bg-[rgba(92,49,30,0.1)] text-[rgba(43,29,20,0.7)]"}`
      ...
      function ProfileBuilder({ onDismiss, initialOptions }) {
        ...
        useEffect(() => {
          if (initialOptions.profile) {
            ...
            setProfileName(name || "");
            setDescription(desc || "");
            if (graph_state == null ? void 0 : graph_state.priorities) {
              setLeftNodes(graph_state.priorities);
            }
            if (graph_state == null ? void 0 : graph_state.families) {
        ...
        const updateFamilyToggle = (enabled) => {
          setFamilies(
            (prev) => prev.map(
              (f) => f.id === drawerFamilyId ? { ...f, enabled } : f
            )
          );
        };
        const updateBlacklist = (model) => {
          setFamilies(
            (prev) => prev.map((f) => {
              ...
              return {
                ...f,
                blacklist: blacklisted ? f.blacklist.filter((m) => m !== model) : [...f.blacklist, model]
              };
            })
          );
        };
